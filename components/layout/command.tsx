...
export function CommandBox({commands, classes}: {commands: Command[], classes: Class[]}) {
        ...
        <CommandList>
          ...
          {classes && classes.length > 0 ? (
            <CommandGroup>
                {classes.map((item)=> {
                    return (
                        <CommandItem onSelect={()=> {redirect(`/class/${item.id}`)}} className="hover:cursor-pointer" key={item.id}>
                        <Avatar>
                            <AvatarImage src={``}/>
                        </Avatar>
                        <span>{item.name}</span>
