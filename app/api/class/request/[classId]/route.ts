...
export async function POST(req: NextRequest, {params}: {params: {classId: string}}){
    ...
    if (dbUser.role !== "student"){
        return new Response("You are not a student, make sure you set up your account correctly", {status: 401});
    }
    if (!currentClass){
    ...
    if (!teacher.email){
        return new Response("Teacher has no email, cannot notify them. Make sure to contact them directly.", {status: 404});
    }
    if (!dbUser.name || !dbUser.email){
        return new Response("You have no name or email provided, make sure you set up your account correctly", {status: 401});
    }
    const res = await sendRequest({
        action_url: `https://www.wordshare.tech/class/${params.classId}/invite?q=${dbUser.name.replaceAll(" ", "%20")}`,
