...
export async function GET(req: NextRequest, { params }: { params: { classId: string } }){
    ...
    if (!classId){
        const placeholder = await fetch("https://placehold.co/600x400/png?text=Icon");
        return new Response(await placeholder.blob(), {status: 200})
    }
...
export async function POST(req: NextRequest, {params}: {params: {classId: string}}){
    ...
    const res = await supabase.storage.from("class").upload(`${classId}/icon`, banner, {upsert: true});
